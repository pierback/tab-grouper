...
export function createTidySnapshot({ windowId, tabs, groups, changedTabIds, appliedGroups, settings }) {
...
export function createUndoPlan(snapshot, currentTabs) {
  ...
  const undoableChangedTabIds = changedTabIds.filter((tabId) => {
    ...
    return appliedGroupIds.size === 0 || appliedGroupIds.has(currentTab.groupId);
  });
