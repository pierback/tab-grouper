...
export async function createGroupPlan(tabs, settings) {
  if (settings.provider === "heuristic") {
    return groupTabsHeuristically(tabs, settings);
  }

  if (settings.provider === "chrome-ai") {
    return createPlanWithChromeAI(tabs, settings);
  }

  if (settings.provider === "local-codex-cli") {
    return createPlanWithNativeCli(tabs, settings, "codex");
  }

  if (settings.provider === "local-claude-cli") {
    return createPlanWithNativeCli(tabs, settings, "claude");
  }

  if (settings.provider === "openai") {
    return createPlanWithOpenAI(tabs, settings);
  }

  if (settings.provider === "anthropic") {
    return createPlanWithAnthropic(tabs, settings);
  }

  throw new Error(`Unknown provider: ${settings.provider}`);
}

export async function createGroupPlanWithFallback(tabs, settings, localTabs = tabs) {
  if (settings.provider === "heuristic") {
    return {
  ...
  try {
    return {
      plan: await createGroupPlan(tabs, settings),
...
async function createPlanWithChromeAI(tabs, settings) {
...
async function createPlanWithOpenAI(tabs, settings) {
...
async function createPlanWithAnthropic(tabs, settings) {
...
function createProviderError(kind, message) {
