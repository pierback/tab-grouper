...
```sh
npm run native:install -- --extension-id <id-from-chrome-extensions>
```
...
```sh
npm run native:install -- --extension-id <id-from-chrome-extensions> --codex-path /path/to/codex --claude-path /path/to/claude
```
